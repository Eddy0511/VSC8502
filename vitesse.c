...
static int vsc8502_config(struct phy_device *phydev)
{
	u32 val;
	u32 timeout = 1000000;
	phy_write(phydev, 0x0, 0x90C0);
	/* Change General Purpose Registers */
	phy_write(phydev, PHY_EXT_PAGE_ACCESS, 0x00);
	...
	phy_write(phydev, PHY_EXT_PAGE_ACCESS, 0x02);
	/* Set Value for RGMII Skew (skew 100:2.0 ns delay)*/
	phy_write(phydev, 0x14, 0x0033); 
