...
#define PHY_EXT_PAGE_ACCESS_EXTENDED1	0x1
#define PHY_EXT_PAGE_ACCESS_EXTENDED2	0x2
#define PHY_EXT_PAGE_ACCESS_EXTENDED3	0x3
#define MIIM_VSC8574_PAGE_ACCESS_GENERAL	0x0
...
/* Vitesse VSC8552 gerneral purpose register */
#define MIIM_VSC8552_MAC_SERDES_CON     0x10
#define MIIM_VSC8552_GENERAL18		0x12
#define MIIM_VSC8552_GENERAL19		0x13
#define MIIM_VSC8552_GENERAL23		0x17

#define MIIM_VSC8552_18G_SGMII		0x80f0
#define MIIM_VSC8552_18G_QSGMII		0x80e0
#define MIIM_VSC8552_18G_CMDSTAT	0x8000
...
#define MIIM_VSC8502_MMCR	0x0
#define MIIM_VSC8502_MAC_SERDES_ANEG	0x80
#define MIIM_VSC8502_GENERAL18		0x12
#define MIIM_VSC8502_GENERAL19		0x13
...
{
	printf("______________________>cis8204\n");
	/* Override PHY config settings */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_CIS82xx_AUX_CONSTAT,
			MIIM_CIS82xx_AUXCONSTAT_INIT);
...
/* Modify by Eddy, 2023/12/20 */
static int vsc8502_config(struct phy_device *phydev)
{
	u32 val;
	u32 timeout = 1000000;
	phy_write(phydev, phydev->addr, 0x0, 0x90C0);
	printf("<Eddy, RGMII> vsc8502_config ADDR[0x%x], ID[0x%x]\n", phydev->addr, phydev->phy_id);
	/* Change General Purpose Registers */
	phy_write(phydev, phydev->addr, PHY_EXT_PAGE_ACCESS, 0x00);
		
	/* Configure RGMII Mode */
	val = phy_read(phydev,phydev->addr,0x17);
	/* set bit 12 '1' for RGMII mode */
	val |= (1<<12);
	phy_write(phydev,phydev->addr,0x17,val);
		
	/* Change General Purpose Registers */
	phy_write(phydev,phydev->addr,PHY_EXT_PAGE_ACCESS,0x00);
	val=phy_read(phydev,phydev->addr, 0x00);
	val|=0x8000;
	phy_write(phydev,phydev->addr,0x00,val);
	/* When bit 15 is cleared the command has completed */
	...
	/* Set RGMII (optional) */
	/* Change Extended Register 2(0x0002) */
	phy_write(phydev, phydev->addr, PHY_EXT_PAGE_ACCESS, 0x02);
	/* Set Value for RGMII Skew (skew 100:2.0 ns delay)*/
	phy_write(phydev, phydev->addr, 0x14, 0x0033); 
	...
	phy_write(phydev, phydev->addr, 0x0, 0x90C0);
	phy_write(phydev, phydev->addr, 0x0, 0x10C0);
		
	printf("<Eddy, RGMII> PHY configure successed\n");
	genphy_config_aneg(phydev);
